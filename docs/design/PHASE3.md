...
If the artist doesn't capture an icon, the preset saves without one and the saved-presets drawer renders a **hash-derived identicon** via `Brushes::IdenticonGen::for_string(name)` — a small generated 64×64 tile (GitHub-style 5×5-mirrored grid + HSV color, all derived from the same name hash). Same name always gets the same identicon; different names look different at a glance. Same identicon is used if the icon PNG sidecar is missing for any reason (artist hand-deleted it, filesystem corruption, etc.) — preset still loads, just renders with the identicon. Pure compute, no fallback PNG asset needed.
...
NEW src/Brushes/IdenticonGen.{hpp,cpp}
                                       Hash-derived 64×64 RGBA identicon
                                       generator. ~50 LOC. for_string(name)
                                       returns an SkBitmap. GitHub-style
                                       5×5-mirrored grid + HSV color, both
                                       derived from std::hash of the name.
                                       Used by A2 drawer for user presets
                                       without a captured icon AND when an
                                       icon PNG sidecar is missing on disk.
                                       No external dep, no fallback asset.
...
src/Screens/PhoneDrawingProgramScreen.cpp (phone)
                                       ...
                                       avatar tile (GUIStuff::ImageDisplay
                                       with onClick) + floating LayoutElement
                                       popover attached via attachTo=PARENT
                                       (mirrors layer_menu_popup pattern).
                                       Popover holds [cap] / [file] /
                                       [Clear] buttons. Phase 3: only [cap]
                                       and [Clear] wired; [file] renders
                                       disabled with a "future phase"
                                       tooltip. Phone variant moves the
                                       avatar tile into the main-menu
                                       popover instead of inline in the
                                       top bar.
...
- **UI system support for "image with overlay buttons" in the top toolbar.** Resolved during design-pass spike (Decisions log §10 #15): Clay supports the pattern via composition of `GUIStuff::ImageDisplay` (image element with onClick) plus a floating `LayoutElement` popover attached via `attachTo = CLAY_ATTACH_TO_PARENT` (the same pattern `PhoneDrawingProgramScreen::layer_menu_popup` already uses). No fallback needed.
...
(All resolved through design-pass research spikes — see Decisions log §10 #14 and #15. Section retained for the next phase's open items.)
...
| 14 | 2026-05-15 | **Hash-derived placeholders are implemented in-tree, no external library.** New `src/Brushes/IdenticonGen.{hpp,cpp}` produces a 64×64 RGBA `SkBitmap` from a string hash using a GitHub-style 5×5-mirrored grid + HSV color derived from the same hash. ~50 LOC, no new Conan dep. Used by A2 drawer for user presets without a captured icon. The generic `data/icons/brush_presets/_no_icon.png` fallback gets dropped — identicon is always available since it's pure compute. | **Spike outcome (Decisions log #8 / §9 open question 1).** ConanCenter scout found no permissively-licensed C++ identicon library worth pulling in (the popular options are JS/Rust/.NET). The GitHub-style identicon algorithm is well-known and small: hash → bits-to-grid → HSV → render. Inkternity's pattern of small in-tree helpers (cf. `src/crypto/stellar/` Stellar strkey) is the right shape here. Using `std::hash` for the seed is sufficient (we want determinism + visual distinction, not crypto). Eliminates the "scout during A2.M2" task and removes the ambiguity of two fallback paths. |
| 15 | 2026-05-15 | **Clay popover-over-image confirmed; B.M0 spike retired.** Clay supports the avatar-tile popover pattern via composition of two existing primitives already in use: (a) `GUIStuff::ImageDisplay` Element renders a PNG and accepts standard Element callbacks including `onClick`; (b) the floating-popover-attached-to-trigger-element pattern is already used by `PhoneDrawingProgramScreen::layer_menu_popup` et al. — a `LayoutElement` with `.floating = { attachTo = CLAY_ATTACH_TO_PARENT, attachPoints = … }` plus an onClick callback that handles outside-click dismissal. Avatar tile = ImageDisplay-with-onClick; popover = floating LayoutElement child with three buttons. No fallback-to-separate-modal needed. | **Spike outcome (Decisions log #11 / §9 open question prerequisite for B.M0).** Verified by inspection of existing Clay usage in `src/GUIStuff/Elements/ImageDisplay.hpp` and `src/Screens/PhoneDrawingProgramScreen.cpp:310-385` (the layer/main menu popovers). Composition has no special edge cases; same z-index + outside-click-dismiss machinery applies. B.M0 milestone removed; the pattern is documented for B.M2 / B.M3 to consume directly. |
